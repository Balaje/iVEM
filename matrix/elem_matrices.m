...
if(poly_deg == 2)
        ...
        for beta = 1:npolys
            for k = 1:length(verts)
                ...
                H(alpha,beta) = H(alpha,beta) + ...
                    integrate2dtri({centroid, vert, next}, ...
                    quad_polynomials{beta}, quad_polynomials{alpha}, diameter, 6);
            end
        end
    end
    C = H*(G\B);
    ...
    C(1,eldofs) = area;
    projector0 = (H\C);            
    stabilising_term = (eye(eldofs)-D*projector0)'*(eye(eldofs)-D*projector0);
    Me = projector0'*H*projector0 + area*stabilising_term;    
end
end
